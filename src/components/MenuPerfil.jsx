...
export default function MenuPerfil() {
  ...
  return (
    <>
      <button
        onClick={() => set_mostrar_modal(true)}
        className="flex flex-col items-center gap-1 text-gray-400 hover:text-[var(--color-com-text)] group transition-colors focus:outline-none"
        title="Editar perfil"
      >
        {usuario?.user_metadata?.avatar_url ? (
          <img
            ...
            alt="Tu avatar"
            className="w-5 h-5 rounded-full object-cover group-hover:-translate-y-1 transition-transform"
          />
        ) : (
